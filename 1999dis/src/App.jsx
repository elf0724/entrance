...
import { 
  ...
  Layers, Database, BarChart3, Map, MapPin, Camera
} from 'lucide-react';
      ...
      <main ref={exportRef} className="flex-1 w-full max-w-[1920px] mx-auto px-4 md:px-6 py-6 overflow-hidden flex flex-col bg-slate-100">
        ...
        {rawData.length > 0 && (
          <div className="mb-8 mt-2 text-center md:text-left">
            ...
            <p className="text-slate-500 mt-2 text-sm flex items-center justify-center md:justify-start gap-2">
        ...
        {rawData.length > 0 && (
          <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-6 duration-700">
            ...
            {/* 篩選與控制面板：優化寬度配置與卷軸容器 */}
            <div className={`mb-6 flex flex-col lg:flex-row lg:items-center gap-4 ${isExporting ? 'bg-transparent border-transparent' : 'bg-white border-slate-200 shadow-sm'} p-4 rounded-xl border transition-colors`}>
              
              <div className={`flex items-center gap-2 ${isExporting ? 'bg-transparent' : 'bg-slate-50 border border-slate-200'} rounded-lg px-3 py-2 flex-shrink-0`}>
              ...
              {!isExporting && <div className="hidden lg:block w-px h-8 bg-slate-200"></div>}

              {/* ★ 修正標籤被遮擋：確保 flex-1 加上 min-w-0 與寬鬆的捲動區 */}
              <div className="flex-1 flex flex-col sm:flex-row sm:items-center gap-3 overflow-hidden min-w-0 w-full">
                <div className="text-sm font-bold text-slate-500 flex-shrink-0 flex items-center">
                ...
                <div 
                  ...
                  className="flex-1 flex overflow-x-auto pb-1 pr-6 gap-2 cursor-grab active:cursor-grabbing hide-scrollbar"
                >
                  {availableCategories.map((cat) => {
                    ...
                    return (
                      <button
                        ...
                        className={`
                          flex-shrink-0 px-4 py-1.5 rounded-md text-sm font-bold transition-all duration-200 flex items-center gap-1.5 border whitespace-nowrap
